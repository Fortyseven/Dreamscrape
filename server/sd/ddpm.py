...
class UNet(DDPM):
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        x_recon = self.model2(h[:step], emb[:step],
                              x_noisy.dtype, hs_temp, cond[:step])

        for i in range(step, bs, step):

            hs_temp = [hs[j][i:i+step] for j in range(lenhs)]
            x_recon1 = self.model2(
                h[i:i+step], emb[i:i+step], x_noisy.dtype, hs_temp, cond[i:i+step])
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True):
        ...
        def to_torch(x): return x.to(self.cdevice)
        ...
        self.register_buffer1('ddim_sqrt_one_minus_alphas',
